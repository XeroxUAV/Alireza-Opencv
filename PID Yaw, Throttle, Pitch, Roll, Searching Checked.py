...
class WindowDetector:
    def __init__(self):
        ...
        self.search_yaw_speed = 40  # سرعت چرخش نرم
        cv2.namedWindow("HSV")
        # HSV1
    ...
    def perform_search(self, tello):
        ...
        if not hasattr(self, "first_search_done"):
            ...
            self.search_yaw_angle = 0
            self.search_direction = 1  # اول راست بچرخه
            self.throttle_direction = 1
            self.throttle_magnitude = 10  # مقدار اولیه throttle
            self.throttle_time_accumulator = 0  # زمان انباشته شده برای throttle
            self.throttle_speed = self.search_yaw_speed / 2  # نصف سرعت yaw
        ...
        if not self.first_search_done:
            ...
            # حرکت throttle (با نصف سرعت yaw)
            self.throttle_time_accumulator += dt
            # فرض کنیم هر سیکل yaw (رفت و برگشت کامل) مدت زمان مشخصی داره، مثلا بر اساس زاویه و سرعت:
            yaw_cycle_duration = (140 * 2) / self.search_yaw_speed  # زمان یک سیکل کامل yaw (رفت و برگشت)
            throttle_toggle_interval = yaw_cycle_duration * 2  # throttle هر دو سیکل yaw تغییر جهت میده
            ...
            if self.throttle_time_accumulator >= throttle_toggle_interval:
                ...
                # افزایش دامنه throttle وقتی جهت تغییر می‌کنه
    ...
    def process(self, frame, pid_yaw, tello, pid_throttle, pid_roll, pid_forward):
        ...
        if best_window is not None:
            self.no_detection_start_time = None
            self.searching = False
            self.search_yaw_angle = 0  # ریست زاویه
            self.search_direction = 1
        ...
        if best_window is None:
            ...
            elif time.time() - self.no_detection_start_time > 4:  # اگر ۳ ثانیه گذشت
                self.searching = True
...
class DroneController:
    def __init__(self):
        ...
        self.pid_forward = PID(Kp=0.001, Ki=0.0001, Kd=0.005)  # مقادیر اولیه، قابل تنظیم با تست
